...
class API(object):
    ...
    def __signature(self):
        ...
        secret_bytes = self.__api_secret.encode('utf-8')
        signature = hmac.new(secret_bytes, string.encode('utf-8'), digestmod=hashlib.sha256).hexdigest().upper()  # create signature
        return signature

    def __post(self, url, param):  # Post Request (Low Level API call)
        params = urllib.parse.urlencode(param)
        req = urllib2.Request(url, params.encode(encoding='utf_8', errors='strict'), {'User-agent': 'bot-cex.io-' + self.__username})
    ...
    def api_call(self, method, param={}, private=0, couple=''):  # api call (Middle level)
        ...
        return answer.decode()  # generate dict and return
